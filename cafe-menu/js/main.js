...
function addCartNumber() {
  ...
  var localStarter = JSON.parse(sessionStorage.getItem("Starter")) || [],
    ...
    localbeerPackage = JSON.parse(sessionStorage.getItem("Beer Package")) || [],
    localSMeals = JSON.parse(sessionStorage.getItem("Solo Meals")) || [];
...
function allOrdertosessionStorage() {
  ...
  // Solo Meals TEST
  if (sMealsList != "") {
    window.sessionStorage.setItem("Solo Meals", JSON.stringify(sMealsList));
  }
...
// Solo Meals TEST
let sMealsList = JSON.parse(sessionStorage.getItem("Solo Meals")) || [];
let sMealsOrder;

function soloMealsArrayy(clicked_object) {
  ...
  let sMealsResult = sMealsList.every(function (res) {
    return res != dataName;
  });

  if (sMealsResult == true) {
    sMealsOrder = sMealsList.push(dataName);
  ...
  switch (page) {
    case "listed-order.php":
      ...
      var starterorders = JSON.parse(sessionStorage.getItem("Starter")),
        ...
        count = 1,
        // solo meals test
        soloMealsOrders = JSON.parse(sessionStorage.getItem("Solo Meals")),
        // soup
      ...
      // solo meals test
      if (soloMealsOrders != null) {
        sortedArr = soloMealsOrders.sort();
        ...
        for (var i = 0; i < sortedArr.length; i = i + count) {
          ...
          // setting the array with delete button
          document.getElementById("soloMealsQ").innerHTML +=
            "<li class='qtyLi'><span id='count-" +
            convertName +
            "'>" +
            count +
            ...
            '<button class="add-qty-btn qty-button" data-name = ' +
            convertName +
            '  key-name = "soloMeals" onClick="QuantityAdd(this)" ><i class="fa-solid fa-plus plus-icon-qty"></i></button><button class="delete-qty-btn qty-button" data-name = ' +
            convertName +
            '  key-name = "soloMeals" onClick="QuantityDelete(this)" ><i class="fa-solid fa-minus trash-icon-qty"></i></button></li>';
        }

        // change the diplay
        document.getElementById("soloMealsListQ").style.display = "block";
      }
      // soup condition
      if (soupOrders != null) {
      ...
      // solo moeals test
      if (soloMealsOrders == null || soloMealsOrders.length === 0) {
        document.getElementById("soloMealsListQ").style.display = "none";
      }
...
function QuantityDelete(clicked_object) {
  ...
  switch (keyName) {
    ...
    // solo meals test
    case "soloMeals":
      if (soloMealsOrders.includes(dataName)) {
        soloMealsOrders.splice(soloMealsOrders.indexOf(dataName), 1);
      }

      // set the new array that will store in local storage
      window.sessionStorage.setItem(
        "Solo Meals",
        JSON.stringify(soloMealsOrders)
      );
      break;
    case "soup":
...
function QuantityAdd(clicked_object) {
  ...
  // solo meals TEST
  let soloMealsList = JSON.parse(sessionStorage.getItem("Solo Meals")) || [];
  let soloMealsOrder;
  ...
  switch (keyName) {
    ...
    // SOLO MEALS TEST
    case "soloMeals":
      soloMealsOrder = soloMealsList.push(dataName);
      window.sessionStorage.setItem(
        "Solo Meals",
        JSON.stringify(soloMealsList)
      );
      break;
    case "soup":
      soupOrder = soupList.push(dataName);
